c---- neutral drift crypt dynamics
      implicit double precision (a-h,o-z)
c--------------------------------------------------------------------
c
c     program to determine least-square fit to the intestinal clonal data
c     based on neutral drift dynamics using a 2d grid sampling of parameter
c     space. Programme samples though parameter space of the ISC 
c     loss/replacement rate (dltmin to dltmax) and time-offset for induction
c     (dt0min to dt0max in units of weeks)
c     
...
c---- raw data files - adjust for given data set
      open(unit=18,file='yfpcontrol.dat')
c      open(unit=18,file='yfpkrasremote.dat')
c      open(unit=18,file='yfppi3kremote.dat')
c
...
c---- and cumulative distribution
      ...
      dltop2=0.0d0
      dt0op2=0.0d0
c---- obtain experimental normalized clone size distribution (and cumulative)
      call expdat(ns,draw,dpnexp,dcnexp,npoints)
c---- loop through all trial values of loss/replacement rate and offset
         do 600 l1=1,ngrid
         dlt=dltmin+(dltmax-dltmin)*dble(l1-1)/dble(ngrid-1)
            do 550 l2=1,ngrid
            dt0=dt0min+(dt0max-dt0min)*dble(l2-1)/dble(ngrid-1)
c---- reset least-square parameters
            dsq(l1,l2)=0.0d0
            dsq2(l1,l2)=0.0d0
               do 300 it=1,ntime
               dlt1=dlt*(dtime(it)-dt0)
c---- get theoretical distribution for given parameter set
               call cdist(ns,dlt1,dpnbar,dcnbar)
...
c---- output optimal parameters
      write(*,'(2a5,f6.4,a5,f6.4,a5,f6.4)') 'dis ','lsq=',
     +   dsqmin,'lam=',dltop,'to=',dt0op
      write(*,'(2a5,f6.4,a5,f6.4,a5,f6.4)') 'cum ','lsq=',
     +   dsqmin2,'lam=',dltop2,'to=',dt0op2
c---- output optimal fit to the clone size distribution
 ...
 750     continue
c---- output optimal fit to cumulative clone size distribution
...
c---- output least-square measures across parameter space for mathematica
 ...
 800     continue
     ...
     +   FontSize -> 16}, PlotLabel -> "least-square statistic",' 
      write(20,'(a120)') 'FrameLabel -> {"Time delay, T (weeks)", 
     +   "Loss/replacement rate, \[Lambda] (per week)"},'
      write(20,'(a120)') 'PlotLegends -> BarLegend[All, 25],'
      write(20,'(a31,f6.3,a3,f6.4,a5,f6.3,a3,f6.4,a3)') 
     +   'Contours -> 25, DataRange -> {{',
     1   dt0min,',',dt0max,'},{',dltmin,',',dltmax,'}}]'
      close(unit=18)
      close(unit=19)
      close(unit=20)
c---- output fit to averages clone sizes
 ...
 810        continue
            write(19,'(4f7.3)') dtime(it),
     +         davexp,davexp/dsqrt(dble(npoints(it))),davbar
 820     continue
      close(unit=19)            
      stop
      end
...
c
c
      subroutine cdist(Nstem,dlt,dpnbar,dcnbar)
c--------------------------------------------------------------------
c     determine theoretical distributions
c
c     input
c     Nstem - effective stem cell number
c     dlt - time
c
c     output
c     dpnbar - clone size distribution
c     dcnbar - cumulative clone size distribution
c--------------------------------------------------------------------
      implicit double precision (a-h,o-z)
...
c---- loop over k sum values
         do 200 lk=1,Nstem-1
         dfk=(2.0d0*dsin(dpi*dble(lk)/2.0d0/dble(Nstem)))**2.0d0
         d1=(1.0d0-dexp(-dlt*dfk))
     +      *(dcos(dpi*dble(lk)/2.0d0/dble(Nstem)))**2.0d0
         dpn(0)=dpn(0)+2.0d0*d1/dble(Nstem)
         dpn(Nstem)=dpn(Nstem)+dble((-1)**(lk+1))*2.0d0*d1/dble(Nstem)
            do 150 n1=1,Nstem-1
            dpn(n1)=dpn(n1)+2.0d0*dsin(dpi*dble(lk)/dble(Nstem))
     +         *dsin(dpi*dble(lk*n1)/dble(Nstem))*dexp(-dlt*dfk)
     1         /dble(Nstem)
 150        continue
 200     continue
