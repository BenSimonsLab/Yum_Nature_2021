##### Calculating the fractions of mesenchymal cell types in Red2Onco models and Confetti control.
...
mesen_df <- data.frame(cellcount_type=integer(),
...
celltypes <- c("Stromal_cell_1", "Stromal_cell_2", "Stromal_cell_3", "Myofibroblast", "Intestitial_cell", "Endothelial_cell", "Glial_cell")

for (i in celltypes) {
  for (j in samples) {
    tmp_idx <- (colData(R2_sce_Mesen)$sample_name == j) 
    tmp_idx2 <- tmp_idx & (colData(R2_sce_Mesen)$celltype == i)
    ...
    mesen_df <- rbind(mesen_df, tmp_df)
  }
  
}

cellcounttable <- mesen_df
cellcounttable$samplecondition <- factor(cellcounttable$samplecondition, levels = c("CONF", "R2KR", "R2P3"))
cellcounttable$celltype <- factor(cellcounttable$celltype, levels = c("Stromal_cell_1", "Stromal_cell_2", "Stromal_cell_3", "Myofibroblast", "Intestitial_cell", "Endothelial_cell", "Glial_cell"))
...
ggplot(Csummary, aes(x=celltype, y=celltyperatio, fill=samplecondition)) + 
  ...
  ggtitle("Cell type fraction in mesenchymal cells")
