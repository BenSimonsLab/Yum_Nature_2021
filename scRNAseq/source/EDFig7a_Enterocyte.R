##### Comparing priming scores of MT epithelial cells for Confetti and Red2Onco models toward enterocyte lineage.
...
colData(R2MT_sce_epi)['FB_Ent'] <- vector(mode='numeric', length=dim(R2MT_sce_epi)[2])
R2MT_sce_epi$FB_Ent <- fb_short$fb_t4
...
FB_df <- data.frame(FB_score=R2MT_ScTA_sce$FB_Ent, cond_color=R2MT_ScTA_sce$cond_color2, stringsAsFactors = FALSE)
p <- ggplot(FB_df, aes(x=cond_color, y=FB_score, color=cond_color)) + 
  ...
  ggtitle("Priming score of MT epithelial cells for Confetti and Red2Onco models toward enterocyte lineage")
