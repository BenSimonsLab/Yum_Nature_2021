##### Calculating AUC scores of NOTCH signaling for epithelial cells
...
geneset_file33 = '../data/Tian_NOTCHblockingAB_DN_modified.gmt'
geneSets33 <- getGmt(geneset_file33)

tmp_sce <- R2_sce_epi
geneSets_NOTCH <- GeneSetCollection(c(geneSets33))
geneSets_NOTCH <- subsetGeneSets(geneSets_NOTCH, toupper(rownames(tmp_sce))) 
...
cells_AUC_NOTCH <- AUCell_calcAUC(geneSets_NOTCH, cells_rankings, aucMaxRank = ceiling(0.3 * nrow(cells_rankings)))    

tmp_sce <- R2_sce_epi
set.seed(123)
par(mfrow=c(1,2)) 
cells_assignment_NOTCH <- AUCell_exploreThresholds(cells_AUC_NOTCH, plotHist=TRUE, assign=TRUE)
...
for (i in Genesets_name) {
  for (j in samples) {
    ...
    tmp_idx2 <- tmp_idx & unname( assays(cells_AUC_NOTCH)[[1]][i,] > unname(AUCthr[i]) )
...
ggplot(Csummary, aes(x=geneset, y=celltyperatio, fill=samplecondition)) + 
  ...
  ggtitle("GSEA for NOTCH target genes in epithelial cells") + 
  xlab("Gene set") +
