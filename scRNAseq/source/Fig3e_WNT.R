##### Calculating AUC scores of WNT signaling for epithelial cells
##### Author: Seungmin Han (sh906@cam.ac.uk)
##### Last Update: 31/03/2021

#### Ref: R2O_intestine_R_SH_191101.ipynb; R2O_intestine_R_SH_GCB1_191009_4.R
...
geneset_file18 = '../data/geneset_M1428.gmt'
geneSets18 <- getGmt(geneset_file18)

tmp_sce <- R2_sce_epi
geneSets_WNT <- GeneSetCollection(c(geneSets18))
geneSets_WNT <- subsetGeneSets(geneSets_WNT, toupper(rownames(tmp_sce))) 
...
cells_AUC_WNT <- AUCell_calcAUC(geneSets_WNT, cells_rankings, aucMaxRank = ceiling(0.3 * nrow(cells_rankings)))    

tmp_sce <- R2_sce_epi
set.seed(123)
par(mfrow=c(1,2)) 
cells_assignment_WNT <- AUCell_exploreThresholds(cells_AUC_WNT, plotHist=TRUE, assign=TRUE)
...
for (i in Genesets_name) {
  for (j in samples) {
    ...
    tmp_idx2 <- tmp_idx & unname( assays(cells_AUC_WNT)[[1]][i,] > unname(AUCthr[i]) )
...
ggplot(Csummary, aes(x=geneset, y=celltyperatio, fill=samplecondition)) + 
  ...
  ggtitle("GSEA for WNT target genes in epithelial cells") + 
  xlab("Gene set") +
