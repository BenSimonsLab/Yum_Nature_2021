##### Estimating the significance of transcriptome change for immune cells based on cell-cell variability 
...
colData(R2_sce_Immune)["sample_name2"] <- vector(mode = "character", length= dim(R2_sce_Immune)[2])
tmp_idx <- ( colData(R2_sce_Immune)$sample_name %in% c("CONF_2", "CONF_3") )
colData(R2_sce_Immune)$sample_name2[tmp_idx] <- "CONF"
tmp_idx <- ( colData(R2_sce_Immune)$sample_name %in% c("R2KR_1", "R2KR_2", "R2KR_3") )
colData(R2_sce_Immune)$sample_name2[tmp_idx] <- "R2KR"
tmp_idx <- ( colData(R2_sce_Immune)$sample_name %in% c("R2P3_1", "R2P3_2") )
colData(R2_sce_Immune)$sample_name2[tmp_idx] <- "R2P3"
...
cmp_nulldist_SH <- function (tmp_sce, celltype_ofinterest, N){
  TstatNull_df <- data.frame(celltype=character(),
                             #              med_inter_var=numeric(),
                             #              med_intra_var=numeric(),
                             t_stat=numeric(),         
                             condition=character(),
...
celltypes <- names( table( colData(R2_sce_Immune)$celltype ) ) 

for (celltype_ofinterest in celltypes) {
  
  tmp_sce <- R2_sce_Immune 
  tmp_idx <- colData(tmp_sce)$celltype == celltype_ofinterest
  tmp_sce <- tmp_sce[,tmp_idx]
...
tstat_pv_Im_df <- tstat_pv_df
rm(tstat_pv_df)
tstat_pv_Im_df # This is p-value for the significance of transcriptomic change of immune cells between Confetti and Red2Onco models (based on cell-cell variability).
